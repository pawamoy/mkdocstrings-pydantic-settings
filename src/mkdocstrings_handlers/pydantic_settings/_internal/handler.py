# This module implements a handler for Pydantic Settings.
...
from mkdocstrings_handlers.pydantic_settings._internal.config import PydanticSettingsConfig, PydanticSettingsOptions
...
class PydanticSettingsHandler(BaseHandler):
    """The PydanticSettings handler class."""

    name: ClassVar[str] = "pydantic settings"
    """The handler's name."""

    domain: ClassVar[str] = "pydantic settings"
    ...
    def __init__(self, config: PydanticSettingsConfig, base_dir: Path, **kwargs: Any) -> None:
    ...
    def get_options(self, local_options: Mapping[str, Any]) -> HandlerOptions:
        ...
        try:
            return PydanticSettingsOptions.from_data(**options)
        except Exception as error:
            raise PluginError(f"Invalid options: {error}") from error

    def collect(self, identifier: str, options: PydanticSettingsOptions) -> CollectorItem:  # noqa: ARG002
    ...
    def render(self, data: CollectorItem, options: PydanticSettingsOptions) -> str:
...
def get_handler(
    ...
    **kwargs: Any,
    ...
    """Simply return an instance of `PydanticSettingsHandler`.
    ...
    Returns:
        An instance of `PydanticSettingsHandler`.
    """
    base_dir = Path(tool_config.config_file_path or "./mkdocs.yml").parent
    return PydanticSettingsHandler(
        config=PydanticSettingsConfig.from_data(**handler_config),
        base_dir=base_dir,
        **kwargs,
    )
