...
try:
    ...
    if sys.version_info < (3, 10):
        try:
            ...
            _logger.debug(
                ...
                "Deactivating Pydantic validation for PydanticSettings handler options.",
            )
            raise
    ...
    _base_url = "https://mkdocstrings.github.io/mkdocstrings-pydantic-settings/usage"
...
class PydanticSettingsInputOptions:
...
# YORE: EOL 3.9: Replace `**_dataclass_options` with `frozen=True, kw_only=True` within line.
@dataclass(**_dataclass_options)
class PydanticSettingsOptions(PydanticSettingsInputOptions):  # type: ignore[override,unused-ignore]
...
# YORE: EOL 3.9: Replace `**_dataclass_options` with `frozen=True, kw_only=True` within line.
@dataclass(**_dataclass_options)
class PydanticSettingsInputConfig:
    """PydanticSettings handler configuration."""

    # We want to validate options early, so we load them as `PydanticSettingsInputOptions`.
    options: Annotated[
        PydanticSettingsInputOptions,
        _Field(description="Configuration options for collecting and rendering objects."),
    ] = field(default_factory=PydanticSettingsInputOptions)
...
# YORE: EOL 3.9: Replace `**_dataclass_options` with `frozen=True, kw_only=True` within line.
@dataclass(**_dataclass_options)
class PydanticSettingsConfig(PydanticSettingsInputConfig):  # type: ignore[override,unused-ignore]
    """PydanticSettings handler configuration."""
