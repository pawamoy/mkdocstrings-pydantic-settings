# Generate a JSON schema of the PydanticSettings handler configuration.
...
def on_post_build(config: MkDocsConfig, **kwargs: Any) -> None:  # noqa: ARG001
    ...
    @dataclass
    class PydanticSettingsHandlerSchema:
        pydantic_settings: PydanticSettingsInputConfig

    adapter = TypeAdapter(PydanticSettingsHandlerSchema)
    ...
    for field in fields(PydanticSettingsInputConfig):  # type: ignore[arg-type]
        if f"setting-{field.name}" not in autorefs._primary_url_map:
            _logger.warning(f"Handler setting `{field.name}` is not documented")
    for field in fields(PydanticSettingsInputOptions):  # type: ignore[arg-type]
