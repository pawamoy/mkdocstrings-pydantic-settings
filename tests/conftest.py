...
if TYPE_CHECKING:
    ...
    from mkdocstrings_handlers.pydantic_settings.handler import PydanticSettingsHandler
...
def fixture_mkdocs_conf(request: pytest.FixtureRequest, tmp_path: Path) -> Iterator[config.Config]:
    ...
    conf_dict = {
        ...
        "plugins": [{"mkdocstrings": {"default_handler": "pydantic settings"}}],
...
def fixture_handler(plugin: MkdocstringsPlugin, ext_markdown: Markdown) -> PydanticSettingsHandler:
    ...
    Returns:
        A handler instance.
    """
    handler = plugin.handlers.get_handler("pydantic settings")
